...
for amp in AMPS:
    ...
    # Initialize X
    X_amp = np.empty([56, NUM_SAMPLES+2])
    X_phi = np.empty([56, NUM_SAMPLES+2])

    # Set output FFT
    Y1 = dataset.iloc[:, 2].values

    # Initialize real and imag array of output FFT
    Y_amp = np.empty([56, NUM_SAMPLES])
    Y_phi = np.empty([56, NUM_SAMPLES])
    ...
    # Fit model
    print('Fit model')
    model_real.fit(X_amp, Y_amp, batch_size=56, epochs=12)
    model_imag.fit(X_phi, Y_phi, batch_size=56, epochs=12)
    #
