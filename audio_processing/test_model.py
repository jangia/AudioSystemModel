...
model_path = os.path.join(BASE_DIR, 'normal_regression', 'models', 'test_model_random_forest_12000_no_hann')
...
fft_data = fft(chunk[:24000])[:12000]

# plt.plot(abs(fft_data[:1200]), 'g')
...
plt.subplot(4, 1, 1)
plt.plot(chunk[:12000], 'y')
plt.title('Original vs Modeled')
plt.ylabel('Amplitude')

# plt.subplot(4, 1, 2)
# plt.plot(np.unwrap(np.angle(fft_data[:12000])), 'g')
# plt.title('Original vs Modeled')
# plt.ylabel('Amplitude')
#
# plt.subplot(4, 1, 3)
# plt.semilogy(np.unwrap(np.angle(y_pred[:12000])), 'r')
# plt.title('Original vs Modeled')
# plt.ylabel('Amplitude')
...
plt.subplot(4, 1, 3)
plt.semilogy(abs(y_pred[:1200]), 'r')
plt.title('Original vs Modeled')
plt.ylabel('Amplitude')
